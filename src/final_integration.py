...
from models.final_integration import StackNN
...
integration_model = torch.load("params/integration_hope_small.params")

unbalance = torch.FloatTensor([1.0, 4.8])
# use Adam optimizer
optimizer = optim.Adam(integration_model.parameters(), lr=2e-5)
...
for epoch in range(1):
    ...
    for i, data in enumerate(train_loader, 0):
		...
		if i % 50 == 49:    # print every 200 mini-batches
			print('epoch %d, iter %5d, loss: %.6f' % (epoch+1, (i+1), running_loss / 50))
			cur_val_auc = eval()
			print('val auc: %.6f' % cur_val_auc)
			if cur_val_auc > last_val_auc:
				last_val_auc = cur_val_auc
				torch.save(integration_model, "params/integration_hope_small.params")
			# itchat.send(
